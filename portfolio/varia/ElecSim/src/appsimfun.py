...
def GetMonteCarloNormalDistGuess(dMeanl, dSDl):
    ...
    while (bOK == False):
        ...
        px = (1 / (dSDl * (pow(2 * 3.14159, 0.5)))) * \
            exp(-pow(iGuess - dMeanl, 2) / (2 * dSDl * dSDl))
        if (px >= random.random()):
            bOK = True
...
def StartAppliance(sApplianceTypelocall, iMeanCycleLengthlocall, iRestartDelaylocall, iRatedPowerlocall, iStandbyPowerlocall):
    ...
    iCycleTimeLeftlocall = CycleLength(
        sApplianceTypelocall, iMeanCycleLengthlocall)
    # print iCycleTimeLeftlocall
    # Determine if this appliance has a delay after the cycle before it can
    # restart
    iRestartDelayTimeLeftlocall = iRestartDelaylocall
    # Set the power
    iPowerlocall = GetPowerUse(
        sApplianceTypelocall, iRatedPowerlocall, iCycleTimeLeftlocall, iStandbyPowerlocall)
...
def GetPowerUse(sApplianceTypel, iRatedPowerl, iCycleTimeLeftl, iStandbyPowerl):
    ...
    # Some appliances have a custom (variable) power profile depending on the
    # time left
