...
def generate_date_single_day(Dwell: Dict, ResultofOccupancySim: int, bWeekend: bool, iMonth: int, iIrradianceThreshold: int, iRandomHouse: int, lightSensor, addHeader: bool) -> List[List[Union[str, float]]]:
    ...
    oMonthlyRelativeTemperatureModifier = [
        0, 1.63, 1.821, 1.595, 0.867, 0.763, 0.191, 0.156, 0.087, 0.399, 0.936, 1.561, 1.994]

    vSimulationArray = [[0 for j in range(33)] for i in range(1442)]
    # This holds result of computation: fore ach appliance power consumption
    ...
    #    singleapp=9
        ...
        # iStandbyPower, "Rated Power", iRatedPower, "Calib", dCalibration,
        # "UseProfile", sUseProfile, "Restart Delay", iRestartDelay
    ...
    #            print vSimulationArray[iCount][iAppliance]
            ...
            iRatedPower = appsimfun.GetMonteCarloNormalDistGuess(
                iRatedPower, iRatedPower / 10)
            # if iAppliance==testapp: print "PRESENT", "Restart Delay Left: ",
            # iRestartDelayTimeLeft, "Rated Power", iRatedPower
            ...
            while iMinute < 1440:
                ...
                sKey = activitydat.KeyRowId(
                    bweek, iActiveOccupants, sUseProfile)
                # print sKey
                #    ' If this appliance is off having completed a cycle (ie. a restart delay)
                # if iAppliance==testapp: print "Time (min)", iMinute, "Ten
                # Min", iTenMinuteCount, "Occ: ", iActiveOccupants, "key to
                # stats", sKey
                if (iCycleTimeLeft <= 0 and iRestartDelayTimeLeft > 0):
                    ...
                    # print "starts on ", iRestartDelayTimeLeft, " Cycles left
                    # ", iCycleTimeLeft
                    ...
                    if (iActiveOccupants > 0 and sUseProfile != "CUSTOM") or (sUseProfile == "LEVEL"):
                        ...
                        if (sUseProfile != "LEVEL") and (sUseProfile != "ACTIVE_OCC") and (sUseProfile != "CUSTOM"):
                            ...
                            # TODO dActivityProbability =
                            # oActivityStatistics(sKey).Modifiers(iTenMinuteCount)
                            dActivityProbability = activitydat.actid[
                                sKey - 1][2 + iTenMinuteCount]
                            ...
                            dActivityProbability = oMonthlyRelativeTemperatureModifier[
                                iMonth]
                            if (iAppliance == testapp):
                                print("CASE C:")
                        ...
                        if random.random() < (dCalibration * dActivityProbability):
                            ...
                            a = []
                            a = appsimfun.StartAppliance(
                                sApplianceType, iMeanCycleLength, iRestartDelay, iRatedPower, iStandbyPower)
                            #                        print a
    ...
    #                            #print iCycleTimeLeft
                    ...
                    if (iActiveOccupants == 0) and (sUseProfile != "LEVEL") and (sUseProfile != "ACT_LAUNDRY") and (sUseProfile != "CUSTOM"):
                        ...
                        # print iCycleTimeLeft
                        iPower = appsimfun.GetPowerUse(
                            sApplianceType, iRatedPower, iCycleTimeLeft, iStandbyPower)
    ...
    OccLights = [0.000, 1.000, 1.52814569536424,
                 1.69370860927152, 1.98344370860927, 2.09437086092715]
    CummDur = [0.111111111, 0.222222222, 0.333333333, 0.444444444,
               0.555555556, 0.666666667, 0.777777778, 0.888888889, 1]
    ...
    for i in range(1, iNumBulbs + 1):
        ...
        fCalibratedRelativeUseWeighting = - \
            fCalibrationScalar * log(random.random())
    ...
    # waarom? Nu gemiddelde per maand rond de 20.000kwh, we willen rond de
    # 1000kwh
    for i in range(start, end):
